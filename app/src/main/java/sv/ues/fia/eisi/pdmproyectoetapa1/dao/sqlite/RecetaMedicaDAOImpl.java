...
import sv.ues.fia.eisi.pdmproyectoetapa1.dao.sqlite.FarmaciaContrato.EntradaRecetaMedica;
...
public class RecetaMedicaDAOImpl implements RecetaMedicaDAO {
    ...
    private boolean verificarIntegridad(RecetaMedica receta, int operacion) {
        ...
        Cursor cursorReceta = db.query(Tablas.RECETA_MEDICA,
                    null,EntradaRecetaMedica.ID_RECETA_MEDICA + "=?",
                            idReceta,null,null,null);
        Cursor cursorMedico = db.query(Tablas.MEDICO, null,
                EntradaRecetaMedica.ID_MEDICO + "=?", idMedico, null, null, null);
    ...
    private static ContentValues getContentValues(RecetaMedica obj){
        ...
        valores.put(EntradaRecetaMedica.NUMERO_RECETA, obj.getNumeroReceta());
        valores.put(EntradaRecetaMedica.FECHA_RECETA_MEDICA, obj.getFechaReceta());
        valores.put(EntradaRecetaMedica.ID_MEDICO, obj.getIdMedico());
        return valores;
    }
    ...
    private static ContentValues getContentValues(RecetaMedica obj, String idReceta){
        ContentValues valores = new ContentValues();
        valores.put(EntradaRecetaMedica.ID_RECETA_MEDICA, idReceta);
        valores.put(EntradaRecetaMedica.NUMERO_RECETA, obj.getNumeroReceta());
        valores.put(EntradaRecetaMedica.FECHA_RECETA_MEDICA, obj.getFechaReceta());
        valores.put(EntradaRecetaMedica.ID_MEDICO, obj.getIdMedico());
        return valores;
    ...
    public void modificar(RecetaMedica obj) throws DAOException {
        ...
        db.update(Tablas.RECETA_MEDICA, valores, EntradaRecetaMedica.ID_RECETA_MEDICA + "=?", idReceta);
    }
    ...
    public void eliminar(RecetaMedica obj) throws DAOException {
        ...
        SQLiteDatabase db = baseDatos.getWritableDatabase();
        String whereClause = String.format("%s=?", EntradaRecetaMedica.ID_RECETA_MEDICA);
        String[] idReceta = {obj.getIdReceta()};
    ...
    public List<RecetaMedica> obtenerTodos() throws DAOException {
        ...
        try(Cursor cursor = db.rawQuery(sql, null)) {
            ...
            do {
                ...
                int idIndex = cursor.getColumnIndex(EntradaRecetaMedica.ID_RECETA_MEDICA);
                int numeroRecetaIndex = cursor.getColumnIndex(EntradaRecetaMedica.NUMERO_RECETA);
                int fechaRecetaIndex = cursor.getColumnIndex(EntradaRecetaMedica.FECHA_RECETA_MEDICA);
                int idMedicoIndex = cursor.getColumnIndex(EntradaRecetaMedica.ID_MEDICO);
    ...
    public RecetaMedica obtener(String id) throws DAOException {
        SQLiteDatabase db = baseDatos.getReadableDatabase();
        String selection = String.format("%s=?", EntradaRecetaMedica.ID_RECETA_MEDICA);
